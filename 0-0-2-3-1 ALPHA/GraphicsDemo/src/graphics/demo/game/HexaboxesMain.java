...
//	The intro part only takes a couple of seconds and isn't skippable
    ...
    public void update(float deltaTime)
    {
        ...
        switch (mode)
        {
            ...
            case 1 :
                ...
                break;
            case 2 :
                boolean touchUp = findTouchUpBoolean(touchEvents);
                if (touchUp)
                {
            ...
            case 3 :
                ...
                boolean touchDownMove = findTouchDownBoolean(touchEvents);
                if(touchDownMove)
                {
                ...
                break;
            case 4 :
                boolean touchUpMove = findTouchUpBoolean(touchEvents);
                if(touchUpMove)
                {
    ...
    private int getUserMenu (TouchEvent touchUpEvent)
    {
        ...
        else if(touchEventCheckInBounds(touchUpEvent, 0, height / 6, width, 2 * height / 6))
        {
            nUsers = 2;
        }
        else if(touchEventCheckInBounds(touchUpEvent, 0, 2 * height / 6, width, 3 * height / 6))
        {
            nUsers = 3;
        }
        else if(touchEventCheckInBounds(touchUpEvent, 0, 3 * height / 6, width, 4 * height / 6))
        {
            nUsers = 4;
        }
        else if(touchEventCheckInBounds(touchUpEvent, 0, 4 * height / 6, width, 5 * height / 6))
        {
            nUsers = 5;
        }
        else if(touchEventCheckInBounds(touchUpEvent, 0, 5 * height / 6, width, height))
        {
            nUsers = 5;
        }
        return nUsers;
    ...
    private static boolean touchEventCheckInBounds(TouchEvent touchEvent, int x, int y, int width, int height) {
